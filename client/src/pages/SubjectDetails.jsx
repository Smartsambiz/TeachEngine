    ...
    return (
        <div className="overflow-x-hidden p-4 sm:p-6 lg:p-10">
            ...
            {error ? (
                ...
                <div className="mt-8 grid gap-4 sm:grid-cols-2">{schemes.map(sch => <div key={sch.id} className="workspace-card p-6"><Link className="workspace-card-interactive block no-underline" to={`/schemes/${sch.id}`}><p className="eyebrow">Scheme of work</p><p className="mt-3 text-lg font-semibold text-slate-900">{sch.term}</p><p className="mt-2 text-sm text-slate-500">{sch.academic_year}</p><p className="mt-5 text-sm font-semibold text-indigo-600">Open scheme →</p></Link><div className="mt-4 flex gap-3 border-t border-slate-100 pt-4"><button type="button" onClick={() => setEditingScheme(sch)} className="text-sm font-semibold text-slate-500 hover:text-indigo-700">Edit</button><button type="button" onClick={() => setDeleteTarget(sch)} className="text-sm font-semibold text-red-600 hover:text-red-700">Delete</button></div></div>)}</div>
            }
